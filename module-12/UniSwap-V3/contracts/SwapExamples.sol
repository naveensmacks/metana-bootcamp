...
contract SwapExamples {
    ...
    //got swapRouter deployed address from uniswapDocs- https://docs.uniswap.org/contracts/v3/reference/deployments
        ...
        returns (uint amountOut)
    {
        ...
        ISwapRouter.ExactInputParams memory params = ISwapRouter
            .ExactInputParams({
                //check the poolFee in here https://info.uniswap.org/#/pools/
                //poolFee(4 decimals) should be exact from the site or it will revert without a reson string
                //In below Example we can use two pool ETH/USDC - 0.3% or ETH/USDC - 0.05%
                path: abi.encodePacked(WETH9, uint24(500), USDC, uint24(100), DAI),
                recipient: msg.sender,
                deadline: block.timestamp,
                amountIn: amountIn,
                amountOutMinimum: 0
        ...
        returns (uint amountIn)
    {
        ...
        ISwapRouter.ExactOutputParams memory params = ISwapRouter
            .ExactOutputParams({
                //unlike swapExactInputMultihop here the path must be in reverse order
        ...
        //transfer back the remaining weth
        if (amountIn < amountInMaximum) {
            TransferHelper.safeApprove(WETH9, address(swapRouter), 0);
            TransferHelper.safeTransferFrom(
                WETH9,
